...
% ===================== 2. 定义数字增益系数 =====================
% 定点化增益系数
gain_factor = uint32(round(1.8 * 2^8));  % 增益系数：1.8（增强亮度），可改为 0.5（减弱）


% ===================== 3. 应用数字增益 =====================
img_gain_double = uint32(uint32(img_gray) * gain_factor);

img_gain_double = round(img_gain_double / (2 ^ 8));

% 截断超出 0-1 范围的值（归一化后）
img_gain_double(img_gain_double > 255) = 255;  % 上限1（对应255）
img_gain_double(img_gain_double < 0)   = 0;  % 下限0（对应0）1

% ===================== 4. 转回 uint8 类型（可选）=====================
img_gain_uint8 = uint8(img_gain_double);

img_gain_uint8_row = img_gain_uint8'
img_gain_uint8_1d = img_gain_uint8_row(:)
fid = fopen('Barbara_gray_pixels_gain_fixed.txt', 'w');  % 新建/覆盖文件
if fid == -1
    error('无法创建文件，请检查路径权限');
end
